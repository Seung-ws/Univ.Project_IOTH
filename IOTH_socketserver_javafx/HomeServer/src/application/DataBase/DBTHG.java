...
public class DBTHG extends DBConnector {
	...
	public void temp_hue_dbadder(Date date,String temp,String hum, String gas) {
	...
	public JSONArray time7list() {
		tryConnect();
		JSONArray jsonArr=new JSONArray();
		try {
		String query="select * from " + 
				"(select * from " + 
				"(select t1.adate,t1.temp,t1.hum,t1.gas " + 
				" from homeserverthg t1, " + 
				"(select bdate,max(adate) as num from homeserverthg group by bdate order by bdate) t2 " + 
				"where t1.adate=t2.num order by t1.adate desc) where rownum<7) order by rownum desc";
		pstmt=con.prepareStatement(query);
		ResultSet rs=pstmt.executeQuery();
